...
得益于极致的性能优化，Genie-TTS 现已支持**流式响应 API** 和**高品质无损变速**：

* **分块流式传输**：基于 FastAPI 提供的流式端点 (`/tts`)，可将音频块（Chunks）实时推送给前端，极大降低了首句延迟。
* **工业级时间拉伸**：后端集成了 Spotify 开源的 `pedalboard` (基于 Rubberband)，实现了真正“不变调、不发虚、无电音”的高级变速支持。
* **划词朗读扩展**：欢迎使用我们配套的 [Genie TTS 浏览器扩展](https://github.com/eacheat53/Genie-TTS-Extension)，一键接入本地服务，体验沉浸式的右键划词听书和热交互变速。
